<x-app-layout>
    ...
    <div class="max-w-2xl mx-auto py-8 animate-in zoom-in duration-500">
        <!-- Main Control Card -->
        <div class="glass-card rounded-[3rem] shadow-premium border border-white/50 overflow-hidden relative group transition-all hover:shadow-premium-hover">
            ...
            <div class="p-12 relative z-10">
                <div class="text-center mb-12">
                    <div class="relative inline-block mb-8">
                        <div class="w-28 h-28 bg-gradient-to-tr from-primary-600 to-primary-500 rounded-[2rem] flex items-center justify-center mx-auto shadow-2xl shadow-primary-200 relative z-10">
                    ...
                    <h3 class="text-4xl font-black text-gray-800 tracking-tight mb-4">Eksperimen Pakan</h3>
                    <p class="text-gray-500 font-bold text-sm tracking-wide">Kirim perintah manual ke alat secara real-time.</p>
                </div>

                @if ($device && $device->status == 'offline')
                    <div class="mb-10 p-6 bg-primary-50/50 backdrop-blur-sm rounded-3xl border border-primary-100 flex items-start gap-5 shadow-sm">
                ...
                <form action="{{ route('control.trigger') }}" method="POST" class="space-y-12" id="manual-feed-form">
                    @csrf
                    <div class="space-y-6">
                        <div class="flex items-center justify-between px-4">
                            <label class="text-xs font-black text-gray-400 uppercase tracking-[0.2em]">Durasi Putaran Servo</label>
                        ...
                        <div class="relative px-2">
                            <input type="range" name="duration" min="1" max="10" step="1" value="{{ $defaultDuration }}" 
                                   class="w-full h-3 bg-gray-200 rounded-full appearance-none cursor-pointer primary-primary-600 focus:outline-none transition-all shadow-inner border border-white/50"
                                   oninput="document.getElementById('duration-display').innerText = this.value + ' DETIK'">
                    ...
                    <button type="submit" id="submit-btn"
                            class="group relative w-full overflow-hidden bg-primary-600 hover:bg-primary-700 text-white font-black py-6 rounded-[2.5rem] shadow-premium transition-all active:scale-95 flex items-center justify-center group border border-primary-500">
            ...
            <!-- Footer Section -->
            <div class="bg-gray-50/50 backdrop-blur-sm px-12 py-10 border-t border-white/30">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div class="flex items-center gap-4 group">
                        <div class="w-10 h-10 rounded-xl bg-primary-50 flex items-center justify-center shadow-inner group-hover:bg-primary-500 group-hover:text-white transition-all">
    ...
    <script>
        document.getElementById('manual-feed-form').addEventListener('submit', function(e) {
            ...
            span.innerText = 'MENGIRIM PERINTAH...';
        });
    </script>
</x-app-layout>
